...
Composio OAuth flows. For connections to appear as **linked** in the dashboard after OAuth success, all of the following are required:

- **Supabase migrations** — Run `20260318_composio_connections.sql` and `20260319_composio_connections_multi_account.sql` so the `composio_connections` table exists with the correct unique index.
- **`SUPABASE_SERVICE_ROLE_KEY`** — Set in the environment. The OAuth callback has no user session (Composio redirect); it identifies the user from a cookie and writes with the service-role client. Without this key, the callback cannot persist the connection and the dashboard will not show it as linked.
- **`COMPOSIO_API_KEY`** — Set in the environment (or in Supabase `app_settings`). Required to generate connect links and list auth configs.
- **Start the flow from this app** — The user must click "Connect" in the dashboard or Mission Control (or use a link generated by the bot) so the callback URL points here and the `composio_cb` cookie is set for user identification.
