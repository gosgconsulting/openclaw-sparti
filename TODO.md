...
- **Supabase data layer refactor (2026-03-20)** — Centralised all Supabase access into `src/lib/db.js`. (1) `getAdminClient()` — lazy singleton service-role client (was re-created on every call). (2) `getUserClient(req)` — replaces the 35 inline `createSupabaseClient({ accessToken: req.supabaseAccessToken })` calls in `mission-control.js`. (3) `getClientForRequest(req)` + `scopeToUser(query, req)` — replaces the local helpers in `sparti-context.js`. (4) `getAppSetting(key, ttlMs)` — cached app_settings reader (10-min TTL) shared by both config resolvers. (5) `getLlmGatewayConfig()` / `resolveComposioApiKey()` — migrated from inline `server.js` helpers; `server.js` now delegates to these. (6) `src/supabase.js` made graceful: both factories return `null` instead of throwing when env vars are absent. Scope: `src/lib/db.js` (new), `src/supabase.js`, `src/server.js`, `src/routes/mission-control.js`, `src/routes/sparti-context.js`. No schema changes; no migrations needed.

- **Unify Tasks database: mc_boards → projects (2026-03-20)** — `mc_boards` was a duplicate of the Sparti `projects` table (same concept: a workspace container for tasks). Migration `20260320_unify_tasks_projects.sql`: (1) Adds `project_id uuid REFERENCES projects(id) ON DELETE CASCADE` to `mc_tasks`. (2) Populates `project_id` for tasks whose board already had `sparti_project_id`. (3) For boards without a linked project, creates a new `projects` row from the board's data (name→title, sparti_brand_id→brand_id) and maps tasks to it. (4) Makes `project_id` NOT NULL. (5) Drops the old `board_id` FK and makes `board_id` nullable (vestigial). (6) Drops `mc_boards` entirely. Code: `src/routes/mission-control.js` board CRUD routes now read/write from `projects` with a `normaliseProject()` shim that preserves the legacy `name`/`sparti_brand_id` field names for the frontend; task routes use `project_id` instead of `board_id`, returning both `project_id` and `board_id` alias for compatibility. No frontend changes required. **Apply migration**: run `20260320_unify_tasks_projects.sql` against Supabase before deploying.

- **Mission Control menu restructure (2026-03-20)** — Full sidebar navigation overhaul. (1) "Live feed" renamed to "History" — panel now loads `bot_sessions` from Supabase across all brands, with a brand filter dropdown and clickable session detail view; new `GET /mission-control/api/history` endpoint. (2) Board groups, Boards, Tags, Approvals, Custom fields consolidated into a single "Tasks" nav item — kanban board list is the default view, board groups/tags/approvals/custom fields accessible via a ⚙ Settings button that opens a tabbed modal. (3) Under Skills: "Packs" replaced by "Agents" (reuses existing `mc_agents` panel connected to `ai_agents`/`custom_agents` tables); "Prompts" moved from Automation to Skills section. (4) "Integrations" moved from its own sidebar section to Overview (below Dashboard). (5) Under Administration: "Organization" renamed to "Brands" — shows brands list from Sparti `brands` table with logo, industry, and active status; "Agents" removed from Admin; new "Logs" entry shows token usage history from `global_ai_token_usage` with summary stats (total tokens, cost) and per-request table. New API routes: `GET /mission-control/api/brands`, `GET /mission-control/api/logs`, `GET /mission-control/api/history`.

- **Token usage wired to existing Supabase tables (2026-03-20)** — Switched from the new `instance_token_usage` table to the existing `global_ai_token_usage` table (used by the main Sparti SaaS). Two changes in `src/server.js`: (1) Added `POST /api/usage` endpoint (SETUP_PASSWORD Bearer auth) — bot pushes per-request data with `{ model, input_tokens, output_tokens, total_tokens, estimated_cost_usd, source, session_id, instance_id }`; rows stored with `provider='openclaw'`, `metadata.source='openclaw'`, `user_id` from `x-user-id` header or `SPARTI_USER_ID` env. (2) `/lite/api/usage` now falls back to reading from `global_ai_token_usage` (admin client, 7-day window, aggregated by date) when gateway RPC and CLI both produce no data — so the usage chart works even when the gateway is stopped. README updated: Supabase persistence model and Bot API tables.

- **Supabase missing tables audit + migrations (2026-03-20)** — Implemented the full "Supabase Missing Tables" plan to close per-user/per-instance data gaps for dashboard + bot + external platform reads. Added migrations: (1) `20260320_app_settings.sql` (`app_settings` table with RLS + updated_at trigger for server-side shared config keys like `llm_gateway` and `composio`), (2) `20260320_instances_bot_columns.sql` (extends `instances` with `railway_service_id`, `railway_env_id`, `gateway_url`, `supabase_url`, `bot_connected_at`, `bot_version`), (3) `20260320_bot_sessions.sql` (new per-user/per-instance session ledger with RLS + indexes), (4) `20260320_instance_token_usage.sql` (reserved for future aggregate/rollup use; primary storage is now `global_ai_token_usage`). README updated with the Supabase App Settings section and Supabase persistence model.

- **Composio DB schema alignment (2026-03-19)** — The `composio_connections` table existed in Supabase but was created from an older, incompatible schema version. Two migrations applied via MCP: (1) `composio_connections_align_schema` — renamed `toolkit_slug` -> `toolkit_key`, `connection_id` -> `connected_account_id`, added `connection_request_id`, dropped the stale single-account unique constraint, created the multi-account unique index `(user_id, toolkit_key, connected_account_id) WHERE connected_account_id IS NOT NULL`, restored performance indexes, wired `updated_at` trigger. (2) `composio_connections_nullable_connected_account_id` — made `connected_account_id` nullable so initiated rows can be inserted before OAuth completes. This was the root cause of the `db_error` failure path.
...
- **Wire `instance_id` into audit emits** — `emitAudit` supports `instance_id` but many callers still pass `null`. Add instance resolution in `server.js` and `src/routes/sparti-context.js` so `mc_audit_events` can be joined cleanly to instance-scoped usage/session records.
- **Add bot heartbeat** — Write `instances.bot_connected_at`/`bot_version` periodically from the gateway boot or health check path so Mission Control can show bot liveness.
- **Wire `POST /api/usage` from bot** — The `sparti-context` skill (or a new `usage-tracker` skill) should call `POST /api/usage` after each agent chat/launch response, passing `input_tokens`, `output_tokens`, `model`, `session_id`, and `instance_id`. `user_id` is auto-resolved account-based from the `instances` table (no env var needed). This gives Mission Control and the Sparti SaaS a shared durable cost log.
...
- **Test connect flow end-to-end**: click Connect on a connector card → should redirect to Composio (e.g. `platform.composio.dev/link/...`) → complete OAuth → should land back on `/dashboard#tab=connectors` or Mission Control with the card showing "connected". With `returnTo=/connectors`, final URL can be `/connectors?status=success&connected_account_id=ca_xxx` then redirect to dashboard with flash.
- **If OAuth succeeds in Composio but dashboard shows "Not connected"**: ensure (1) both Composio migrations are applied, (2) `SUPABASE_SERVICE_ROLE_KEY` is set (callback needs it to upsert without a user session), (3) the link was opened from our app (dashboard/Mission Control or bot) so the callback URL and cookie are correct. See `docs/PLAN.md` section "Composio linked to dashboard (prerequisites and troubleshooting)".
...
- **Composio Google Super + standalone Google connectors (2026-03-19):** (1) Added `googlesuper` and `google_super` to TOOLKIT_META in `server.js` with recommended display name "Google Super" and description (one OAuth for Gmail, Drive, Calendar, Sheets, Docs, Meet, Analytics, Ads, Photos). (2) Connector list now shows **Google Super** as its own card (when present in Composio auth configs) right after Web Search; individual Google product toolkits (gmail, googledrive, googlesheets, googleads, etc.) remain grouped under **Google Workspace (per service)** so users can connect either one-click (Google Super) or per-service (e.g. Google Ads standalone). (3) composio-connect skill v1.4.0: auth table and when-to-use updated with Google Super (`googlesuper` / `google_super`) and note that individual connectors (including Google Ads) are available when not using Google Super. (4) README Pre-bundled Skills and Connectors sections updated. To see the Google Super card in the dashboard, add the Google Super app as an auth config in the Composio dashboard (platform.composio.dev).

- **Composio OAuth: no second login (session restore + oc_return) (2026-03-19):** After connecting via Composio OAuth, users were sometimes redirected to `/auth?redirect=...` and asked to log in again because Supabase cookies were not always sent on the return from Composio. Two fixes: (1) **Session restore on callback**: when the user clicks Connect/Reconnect, we store an AES-256-GCM encrypted copy of the refresh token in the `composio_cb` cookie (key from `SETUP_PASSWORD`). When the callback runs after OAuth, we decrypt it, call `refreshSession`, and set Supabase auth cookies on the response before redirecting to Mission Control — so the user lands already logged in. (2) **Hash preservation**: if they do land on `/auth` (e.g. refresh token expired), we set an `oc_return` cookie (path=/, 20 min) with the full return path including hash when they start the connect flow; `requireUser()` uses it when building the redirect-to-auth URL so after login they go to e.g. `/mission-control#integrations`. GET `/auth` clears `oc_return` so it is single-use. Verification: connect from Mission Control → complete OAuth → should land on `/mission-control#integrations&connect=success` without being asked to log in again.
