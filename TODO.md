...
- **Set `SPARTI_USER_ID` env var** — Required for `POST /api/usage` and the `/lite/api/usage` Supabase fallback to associate usage rows with the correct Supabase user. Set the user's UUID in Railway env vars. Without it, rows are stored with `user_id = null` (anonymous) and the usage fallback chart returns nothing.
- **Wire `POST /api/usage` from bot** — The `sparti-context` skill (or a new `usage-tracker` skill) should call `POST /api/usage` after each agent chat/launch response, passing `input_tokens`, `output_tokens`, `model`, `session_id`, and `instance_id` from the gateway response. This gives Mission Control and the Sparti SaaS a shared durable cost log.
