...
- **Add bot heartbeat + usage writes** — Write `instances.bot_connected_at`/`bot_version` periodically from the gateway/bot path and persist usage rows into `instance_token_usage` (or upsert rollups) so Mission Control/external platforms can query durable cost metrics without relying on gateway RPC memory.
